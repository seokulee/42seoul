...
t_data	*init_data(void)
{
	...
	mlx_get_screen_size(new_data->mlx, &new_data->width, &new_data->height);
	new_data->mlx_win = mlx_new_window(new_data->mlx, new_data->width, new_data->height, "miniRT");
	new_data->mlx_img = (t_img *)malloc(sizeof(t_img));
	if (new_data->mlx_img == NULL)
		return (NULL);
	new_data->mlx_img->img = mlx_new_image(new_data->mlx, new_data->width, new_data->height);
